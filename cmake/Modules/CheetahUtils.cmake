...
# Because cheetah spends a lot of time setting up custom compile
# flags, define a handy helper function for it. The compile flags
# setting in CMake has serious issues that make its syntax challenging
# at best.
...
macro(append_rtti_flag polarity list)
  if(${polarity})
    append_list_if(CHEETAH_HAS_FRTTI_FLAG -frtti ${list})
    append_list_if(CHEETAH_HAS_GR_FLAG /GR ${list})
  else()
    append_list_if(CHEETAH_HAS_FNO_RTTI_FLAG -fno-rtti ${list})
    append_list_if(CHEETAH_HAS_GR_FLAG /GR- ${list})
  endif()
endmacro()
...
function(filter_available_targets out_var)
  set(archs ${${out_var}})
  foreach(arch ${ARGN})
    list(FIND CHEETAH_CMAKE_SUPPORTED_ARCH ${arch} ARCH_INDEX)
...
macro(test_target_arch arch def)
  ...
  if(NOT DEFINED CAN_TARGET_${arch})
    ...
    elseif(TEST_COMPILE_ONLY)
      ...
      if(CHEETAH_HAS_FNO_EXCEPTIONS_FLAG)
  ...
  if(${CAN_TARGET_${arch}})
    list(APPEND CHEETAH_CMAKE_SUPPORTED_ARCH ${arch})
  elseif("${CHEETAH_DEFAULT_TARGET_ARCH}" STREQUAL "${arch}" AND
         CHEETAH_HAS_EXPLICIT_DEFAULT_TARGET_TRIPLE)
...
macro(load_llvm_config)
  if (NOT LLVM_CONFIG_PATH)
    ...
    if (NOT LLVM_CONFIG_PATH)
      message(WARNING "UNSUPPORTED CHEETAH CONFIGURATION DETECTED: "
  ...
  if (LLVM_CONFIG_PATH)
    execute_process(
      COMMAND ${LLVM_CONFIG_PATH} "--obj-root" "--bindir" "--libdir" "--includedir" "--host-target"
    ...
    set(LLVM_MAIN_SRC_DIR ${LLVM_BINARY_DIR}/.. CACHE PATH "Path to LLVM source tree")
    ...
    if (HAD_ERROR)
      ...
      set(CHEETAH_HAS_LLVMXRAY FALSE)
    else()
      string(REGEX REPLACE "[ \t]*[\r\n]+[ \t]*" ";" CONFIG_OUTPUT ${CONFIG_OUTPUT})
...
macro(construct_cheetah_default_triple)
  if(CHEETAH_DEFAULT_TARGET_ONLY)
    if(DEFINED CHEETAH_DEFAULT_TARGET_TRIPLE)
      message(FATAL_ERROR "CHEETAH_DEFAULT_TARGET_TRIPLE isn't supported when building for default target only")
    endif()
  ...
  if(DEFINED CHEETAH_TEST_TARGET_TRIPLE)
    ...
    # CHEETAH_TEST_TARGET_TRIPLE.
    set(CHEETAH_DEFAULT_TARGET_TRIPLE ${CHEETAH_TEST_TARGET_TRIPLE})
  endif()

  string(REPLACE "-" ";" TARGET_TRIPLE_LIST ${CHEETAH_DEFAULT_TARGET_TRIPLE})
  list(GET TARGET_TRIPLE_LIST 0 CHEETAH_DEFAULT_TARGET_ARCH)
  # Determine if test target triple is specified explicitly, and doesn't match the
  # default.
  if(NOT CHEETAH_DEFAULT_TARGET_TRIPLE STREQUAL TARGET_TRIPLE)
    set(CHEEETAH_HAS_EXPLICIT_DEFAULT_TARGET_TRIPLE TRUE)
  else()
    set(CHEETAH_HAS_EXPLICIT_DEFAULT_TARGET_TRIPLE FALSE)
  endif()
endmacro()
...
function(get_cheetah_target arch variable)
  string(FIND ${CHEETAH_DEFAULT_TARGET_TRIPLE} "-" dash_index)
  string(SUBSTRING ${CHEETAH_DEFAULT_TARGET_TRIPLE} ${dash_index} -1 triple_suffix)
  if(CHEETAH_DEFAULT_TARGET_ONLY)
    # Use exact spelling when building only for the target specified to CMake.
    set(target "${CHEETAH_DEFAULT_TARGET_TRIPLE}")
...
function(get_cheetah_install_dir arch install_dir)
  if(CHEETAH_ENABLE_PER_TARGET_RUNTIME_DIR AND NOT APPLE)
    ...
    set(${install_dir} ${CHEETAH_LIBRARY_INSTALL_DIR} PARENT_SCOPE)
  endif()
endfunction()
...
function(get_cheetah_output_dir arch output_dir)
  if(CHEETAH_ENABLE_PER_TARGET_RUNTIME_DIR AND NOT APPLE)
    ...
    set(${output_dir} ${CHEETAH_LIBRARY_OUTPUT_DIR} PARENT_SCOPE)
  endif()
endfunction()

# cheetah_process_sources(
#   <OUTPUT_VAR>
#   <SOURCE_FILE> ...
...
# because we need to support standalone builds of cheetah.
function(cheetah_process_sources OUTPUT_VAR)
...
function(add_cheetah_install_targets name)
  ...
  if(ARG_PARENT_TARGET AND NOT TARGET install-${ARG_PARENT_TARGET})
    ...
    set_target_properties(install-${ARG_PARENT_TARGET}-stripped PROPERTIES
                          FOLDER "Cheetah Misc")
    add_dependencies(install-cheetah install-${ARG_PARENT_TARGET})
    add_dependencies(install-cheetah-stripped install-${ARG_PARENT_TARGET}-stripped)
  endif()
